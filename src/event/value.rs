...
impl Value {
    ...
    /// will be prefixed with that lookup.
    #[instrument(level = "trace")]
    pub fn pairs<'values>(&'values self, prefix: Option<Lookup<'values>>, only_leaves: bool) -> Pairs<'values> {
        match &self {
            Value::Boolean(_)
            | Value::Bytes(_)
            | Value::Timestamp(_)
            | Value::Float(_)
            | Value::Integer(_)
            | Value::Null => Pairs::leaf(prefix, self),
            Value::Map(_children) => {
                Pairs::map(prefix, self, only_leaves)
            }
            Value::Array(_children) => {
                Pairs::array(prefix, self, only_leaves)
            }
        }
    }
}
...
pub enum Pairs<'values> {
    Leaf {
        current_prefix: Option<Lookup<'values>>,
        value: &'values Value,
    },
    Map {
        current_prefix: Option<Lookup<'values>>,
        value: &'values Value,
        inner_pairs: Box<Pairs<'values>>,
        only_leaves: bool,
        returned_self: bool,
    },
    Array {
...
impl<'values> Pairs<'values> {
    fn map(current_prefix: Option<Lookup<'values>>, value: &'values Value, only_leaves: bool) -> Self {
        match value {
            Value::Map(_children) => {
                Self::Map {
                    current_prefix: current_prefix.clone(),
                    value,
                    inner_pairs: Box::new(value.pairs(current_prefix, only_leaves)),
                    only_leaves,
                    returned_self: false
                }
            },
            _ => unreachable!("Unreachable codepath in internal Pairs descent. Please report this."),
        }
    }

    fn array(current_prefix: Option<Lookup<'values>>, value: &'values Value, only_leaves: bool) -> Self {
        match value {
            Value::Array(_children) => {
                Self::Array {
                    current_prefix: current_prefix.clone(),
    ...
    fn leaf(current_prefix: Option<Lookup<'values>>, value: &'values Value) -> Self {
        Self::Leaf {
            current_prefix,
            value,
        }
...
impl<'values> Iterator for Pairs<'values> {
    type Item = (Lookup<'values>, &'values Value);

    fn next(&mut self) -> Option<Self::Item> {
        match self {
            Pairs::Map { ref current_prefix, ref value, ref mut inner_pairs, ref only_leaves, ref mut returned_self } => {
                if !*only_leaves && !*returned_self && current_prefix.is_some() {
                    *returned_self = true;
                    current_prefix.clone().map(|l| (l, *value))
                } else {
                    inner_pairs.next().map(|(iter_lookup, value)| {
                        let lookup = current_prefix.clone().map_or_else(|| {
                            iter_lookup.clone()
                        }, |mut lookup| {
                            lookup.extend(iter_lookup.clone());
                            lookup
                        });
                        (lookup, value)
                    })
            ...
            Pairs::Array { ref current_prefix, ref value, ref mut inner_pairs, ref only_leaves, ref mut returned_self } => {
                if !*only_leaves && !*returned_self && current_prefix.is_some() {
                    *returned_self = true;
                    current_prefix.clone().map(|l| (l, *value))
                } else {
            ...
            Pairs::Leaf { ref current_prefix, ref value} => {
                current_prefix.clone().map(|l| (l, *value))
            },
        }
    }
